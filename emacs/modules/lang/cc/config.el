...
(def-package! cc-mode
  ...
  (unless (assoc "nucleus" c-style-alist)
    (push '("nucleus"
...
(def-package! irony
  ...
  :preface
  (setq irony-server-install-prefix (concat nucleus-etc-dir "irony-server/"))
  :init
...
(def-package! rtags
  ...
  :preface
  (setq rtags-install-path (concat nucleus-etc-dir "rtags/"))
  :init
  (defun +cc|init-rtags ()
  ...
  (add-hook 'nucleus-cleanup-hook #'+cc|cleanup-rtags)
