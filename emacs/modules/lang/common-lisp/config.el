...
(after! sly
  (setq sly-mrepl-history-file-name (concat nucleus-cache-dir "sly-mrepl-history")
  ...
  (defun +common-lisp|init-sly ()
    ...
    (cond ((or (nucleus-temp-buffer-p (current-buffer))
               (sly-connected-p)))
...
installed through an external script (bin/nucleus), `load-file-name' is set to
bin/nucleus while packages at compile-time (not a runtime though)."
