...
nimsuggest isn't installed."
  ...
  (when IS-WINDOWS
    ...
    (defun nucleus*nimsuggest--get-dirty-dir ()
...
`nimsuggest--make-tempdir' tries to use it."
    ...
    (advice-add #'nimsuggest--get-dirty-dir :override #'nucleus*nimsuggest--get-dirty-dir)
    
    ;; TODO File PR/report upstream (https://github.com/nim-lang/nim-mode)
    (defun nucleus*nimsuggest--get-temp-file-name (path)
...
windows."
    ...
    (advice-add #'nimsuggest--get-temp-file-name :filter-return #'nucleus*nimsuggest--get-temp-file-name))
