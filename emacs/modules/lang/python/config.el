...
(def-package! python
  :defer t
  :init
  (setq python-environment-directory nucleus-cache-dir
...
(def-package! anaconda-mode
  :hook python-mode
  :init
  (setq anaconda-mode-installation-directory (concat nucleus-etc-dir "anaconda/")
  ...
  (defun +python|auto-kill-anaconda-processes ()
    ...
    (when (and (eq major-mode 'python-mode)
               (not (delq (current-buffer)
                          (nucleus-buffers-in-mode 'python-mode (buffer-list)))))
      (anaconda-mode-stop)))
