...
buffers open."
  ...
  (unless (or (not robe-mode) (nucleus-buffers-in-mode 'enh-ruby-mode))
...
started it."
  (condition-case _
      (let ((version-str (car (process-lines "ruby" "--version"))))
        (puthash (or (nucleus-project-root) default-directory)
...
(defun +ruby|update-version (&rest _)
  ...
  (setq +ruby--version
        (or (gethash (or (nucleus-project-root) default-directory)
                     +ruby-version-cache)
            (+ruby-version))))
...
(defun +ruby|update-version-in-all-buffers (&rest )
  ...
  (dolist (buffer (nucleus-buffers-in-mode 'enh-ruby-mode))
