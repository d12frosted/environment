...
(def-package! tide
  ...
  ;; code completion
  (after! company
    ;; tide affects the global `company-backends', undo this so nucleus can handle
    ;; it buffer-locally
  ...
  ;; resolve to `nucleus-project-root' if `tide-project-root' fails
...
(def-package! js2-refactor
  ...
  (when (featurep! :feature evil +everywhere)
    (let ((js2-refactor-mode-map (evil-get-auxiliary-keymap js2-refactor-mode-map 'normal t t)))
      (js2r-add-keybindings-with-prefix (format "%s r" nucleus-localleader-key)))))
