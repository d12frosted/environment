...
(defvar +popup-buffer-mode-map
  (let ((map (make-sparse-keymap)))
    (when (featurep! :feature evil)
      ...
      (define-key map [escape] #'nucleus/escape))
    map)
...
(define-minor-mode +popup-mode
  ...
  (cond (+popup-mode
         (add-hook 'nucleus-escape-hook #'+popup|close-on-escape t)
         (add-hook 'nucleus-cleanup-hook #'+popup|cleanup-rules)
        ...
        (t
         (remove-hook 'nucleus-escape-hook #'+popup|close-on-escape)
         (remove-hook 'nucleus-cleanup-hook #'+popup|cleanup-rules)
...
(add-hook 'nucleus-init-ui-hook #'+popup-mode :append)
