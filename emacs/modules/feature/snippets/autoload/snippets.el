...
(defun +snippets--remove-p (x y)
  (and (equal (yas--template-key x) (yas--template-key y))
       (file-in-directory-p (yas--template-get-file x) nucleus-emacs-dir)))
...
`nucleus-emacs-dir'; provided by Doom or its plugins) or your private snippets
(outside of `nucleus-eamcs-dir').
...
(defun +snippets/browse (arg)
  "TODO"
  (interactive "P")
  (nucleus-project-browse +snippets-dir))
...
(defun +snippets/find-file ()
  ...
  (if (file-directory-p +snippets-dir)
      (nucleus-project-find-file +snippets-dir)
