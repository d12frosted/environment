...
(def-package! evil
  ...
  :config
  (add-hook 'nucleus-post-init-hook #'evil-mode)
  ...
  (defun +evil|init-popup-rules ()
    (set-popup-rules!
      '(("^\\*evil-registers" :size 0.3)
        ("^\\*Command Line"   :size 8))))
  (add-hook 'nucleus-post-init-hook #'+evil|init-popup-rules)
  ...
  (add-hook 'nucleus-load-theme-hook #'+evil|update-cursor-color)
  ...
  (defun +evil|disable-highlights ()
    ...
    (when (evil-ex-hl-active-p 'evil-ex-search)
      (evil-ex-nohighlight)
      t))
  (add-hook 'nucleus-escape-hook #'+evil|disable-highlights)
  ...
  (defun +evil|display-vimlike-save-message ()
    ...
    (message "\"%s\" %dL, %dC written"
             (if buffer-file-name
                 (file-relative-name (file-truename buffer-file-name) (nucleus-project-root))
               (buffer-name))
  ...
  ;; Make ESC (from normal mode) the universal escaper. See `nucleus-escape-hook'.
...
(def-package! evil-exchange
  ...
  (defun +evil|escape-exchange ()
    (when evil-exchange--overlays
      ...
      t))
  (add-hook 'nucleus-escape-hook #'+evil|escape-exchange))
...
(def-package! evil-vimish-fold
  ...
  :init
  (setq vimish-fold-dir (concat nucleus-cache-dir "vimish-fold/")
