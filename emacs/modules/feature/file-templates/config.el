...
(defvar +file-templates-alist
  `(;; General
    ...
    ("/doctor\\.el$" :when +file-templates-in-emacs-dirs-p
     :trigger "__nucleus-doctor"
     :mode emacs-lisp-mode)
    ("/test/.+\\.el$" :when +file-templates-in-emacs-dirs-p
     :trigger "__nucleus-test"
     :mode emacs-lisp-mode)
...
(defun +file-templates-in-emacs-dirs-p (file)
  ...
  (or (file-in-directory-p file nucleus-private-dir)
...
(after! yasnippet
  (if (featurep! :feature snippets)
    ...
    (add-hook 'nucleus-escape-hook #'yas-abort-snippet)
