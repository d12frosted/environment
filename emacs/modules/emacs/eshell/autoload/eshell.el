...
(defvar eshell-buffer-name "*nucleus eshell*")
...
(defun +eshell/open (arg &optional command)
  ...
  (let* ((default-directory (or (if arg default-directory (nucleus-project-root))
                                default-directory))
...
(defun +eshell/switch-to (buffer)
  ...
  (interactive
   (let ((buffers (nucleus-buffers-in-mode
        ...
        (cond ((and (one-window-p t)
              ...
              ((one-window-p)
               (let ((prev (save-window-excursion (previous-buffer))))
                 (unless (and prev (nucleus-real-buffer-p prev))
                   (switch-to-buffer (nucleus-fallback-buffer)))))
