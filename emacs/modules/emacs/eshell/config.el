...
;;
(defvar eshell-directory-name (concat nucleus-etc-dir "eshell"))
...
(after! eshell ; built-in
  ...
  (add-to-list 'nucleus-detect-indentation-excluded-modes 'eshell-mode nil #'eq)

  ;; Consider eshell buffers real
  (add-hook 'eshell-mode-hook #'nucleus|mark-buffer-as-real)
...
redefines its keys every time `eshell-mode' is enabled."
    ...
    (define-key! eshell-mode-map
      ...
      [remap nucleus/backward-to-bol-or-indent] #'eshell-bol
      [remap nucleus/backward-kill-to-bol-and-indent] #'eshell-kill-input
...
(def-package! eshell-z
  :after eshell
  :config
  ;; Use zsh's db if it exists, otherwise, store it in `nucleus-cache-dir'
