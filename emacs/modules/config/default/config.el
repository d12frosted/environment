...
(defvar auth-sources
  (list (expand-file-name "authinfo.gpg" nucleus-etc-dir)
        "~/.authinfo.gpg"))
...
(after! smartparens
  ...
  (advice-add #'delete-backward-char :override #'nucleus/delete-backward-char)

  ;; Makes `newline-and-indent' smarter when dealing with comments
  (advice-add #'newline-and-indent :around #'nucleus*newline-indent-and-continue-comments))
...
`+default-repeat-backward-key'."
        (let ((fn-sym (intern (format "+evil*repeat-%s" (nucleus-unquote command)))))
          `(progn
             (defun ,fn-sym (&rest _)
