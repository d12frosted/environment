;;; completion/ivy/autoload.el -*- lexical-binding: t; -*-
;;
...
search current file. See `+ivy-task-tags' to customize what this searches for."
  ...
  (ivy-read (format "Tasks (%s): "
                    (if arg
                      ...
                      "project"))
            (let ((tasks (+ivy--tasks (if arg buffer-file-name (+project-root)))))
              (unless tasks
...
(defun +ivy/projectile-find-file ()
  "A more sensible `counsel-projectile-find-file', which will
revert to `counsel-find-file' if invoked from $HOME,
`counsel-file-jump' if invoked from a non-project,
`projectile-find-file' if in a bug project (more than
...
QUERY is a regexp. If omitted, the current selection is used. If
no selection is active, the last known search is used.
...
If ALL-FILES-P, search compressed and hidden files as well."
  ...
  (defalias (intern (format "+ivy/%s-from-cwd" engine))
    ...
    (format "Perform a project file search from the current
    directory using %s.
