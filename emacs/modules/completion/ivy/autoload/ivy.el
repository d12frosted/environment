...
(defun +ivy--tasks-open-action (x)
  ...
  (let ((location (cadr (split-string x " | ")))
    ...
    (cl-destructuring-bind (file line) (split-string location ":")
      (with-ivy-window
        (find-file (expand-file-name file (nucleus-project-root)))
        (goto-char (point-min))
...
search current file. See `+ivy-task-tags' to customize what this searches for."
  ...
  (ivy-read (format "Tasks (%s): "
                    (if arg
                      ...
                      "project"))
            (let ((tasks (+ivy--tasks (if arg buffer-file-name (nucleus-project-root)))))
              (unless tasks
...
The point of this is to avoid Emacs locking up indexing massive file trees."
  ...
  (call-interactively
   (cond ((or (file-equal-p default-directory "~")
              (when-let* ((proot (nucleus-project-root)))
                (file-equal-p proot "~")))
          #'counsel-find-file)
