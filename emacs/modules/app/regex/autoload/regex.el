...
(defface +regex-match-0-face
  `((t (:foreground "Black" :background ,(nucleus-color 'magenta) :bold t)))
  "TODO"
  :group 'faces)

(defface +regex-match-1-face
  `((t (:foreground "Black" :background ,(nucleus-color 'blue) :bold t)))
  "TODO"
  :group 'faces)

(defface +regex-match-2-face
  `((t (:foreground "Black" :background ,(nucleus-color 'green) :bold t)))
  "TODO"
  :group 'faces)
...
(defun =regex (&optional dummy-text)
  ...
  (unless (buffer-live-p +regex--expr-buffer)
    (condition-case ex
        (progn
          (setq +regex--expr-buffer (get-buffer-create "*nucleus-regex*")
                +regex--text-buffer (if dummy-text (get-buffer-create "*nucleus-regex-text*") (current-buffer))
                +regex--groups-buffer (get-buffer-create "*nucleus-regex-groups*"))
          (when dummy-text
  ...
  (unless (buffer-live-p +regex--replace-buffer)
    (let (text)
      ...
      (with-selected-window (get-buffer-window +regex--text-buffer)
        ...
        (switch-to-buffer (get-buffer-create "*nucleus-regex-text-repl*"))
        (erase-buffer)
        (insert text)
        (read-only-mode +1)
      ...
      (with-current-buffer +regex--expr-buffer
        (select-window (split-window-right))
        (switch-to-buffer (get-buffer-create "*nucleus-regex-repl*"))
        (conf-mode)
        (rainbow-delimiters-mode +1)
        (nucleus/toggle-line-numbers -1)
        (setq-local require-final-newline nil)
...
(defun +regex/quit ()
  ...
  (when (and +regex--text-buffer (buffer-live-p +regex--text-buffer))
    ...
    (when (equal (buffer-name +regex--text-buffer) "*nucleus-regex-text*")
