...
(dispatcher! (upgrade up) (nucleus-upgrade)
...
(defvar nucleus-repo-url "https://github.com/hlissner/nucleus-emacs"
  "TODO")
(defvar nucleus-repo-remote "_upgrade"
  "TODO")

(defun nucleus--working-tree-dirty-p (dir)
  (with-temp-buffer
...
(defun nucleus-upgrade ()
  "Upgrade Doom to the latest version non-destructively."
  (require 'vc-git)
  (let* ((gitdir (expand-file-name ".git" nucleus-emacs-dir))
         (branch (vc-git--symbolic-ref nucleus-emacs-dir))
         (default-directory nucleus-emacs-dir))
    ...
    (when (nucleus--working-tree-dirty-p nucleus-emacs-dir)
      (user-error "Refusing to upgrade because Doom has been modified. Stash or undo your changes"))
    (with-temp-buffer
      (let ((buf (current-buffer)))
        (condition-case-unless-debug e
            (progn
              (process-file "git" nil buf nil "remote" "remove" nucleus-repo-remote)
              (unless (zerop (process-file "git" nil buf nil "remote" "add"
                                           nucleus-repo-remote nucleus-repo-url))
                (error "Failed to add %s to remotes" nucleus-repo-remote))
              (unless (zerop (process-file "git" nil buf nil "fetch" "--tags"
                                           nucleus-repo-remote branch))
                (error "Failed to fetch from upstream"))
              (let ((current-rev (vc-git-working-revision nucleus-emacs-dir))
                    (rev (string-trim (shell-command-to-string (format "git rev-parse %s/%s" nucleus-repo-remote branch)))))
                (unless rev
                  (error "Couldn't detect Doom's version. Is %s a repo?"
                         (abbreviate-file-name nucleus-emacs-dir)))
                (when (equal current-rev rev)
                  (user-error "Doom is up to date!"))
                (message "Updates for Doom are available!\n\n  Old revision: %s\n  New revision: %s\n"
                         current-rev rev)
                (message "Comparision diff: https://github.com/hlissner/nucleus-emacs/compare/%s...%s\n"
                         (substring current-rev 0 10) (substring rev 0 10))
                ;; TODO Display newsletter diff
                (unless (or nucleus-auto-accept (y-or-n-p "Proceed?"))
                  (user-error "Aborted"))
                ...
                (unless (zerop (process-file "git" nil buf nil "reset" "--hard"
                                             (format "%s/%s" nucleus-repo-remote branch)))
                  (error "An error occurred while checking out the latest commit\n\n%s"
                         (buffer-string)))
                (unless (equal (vc-git-working-revision nucleus-emacs-dir) rev)
                  (error "Failed to checkout latest commit.\n\n%s" (buffer-string)))
                (nucleus-refresh 'force)
