...
(describe "core/lib"
  ;; --- Helpers ----------------------------
  (describe "nucleus-unquote"
    (it "unquotes a quoted form"
      (expect (nucleus-unquote '(quote hello)) :to-be 'hello))
    (it "unquotes nested-quoted forms"
      (expect (nucleus-unquote '(quote (quote (a b c)))) :to-equal '(a b c)))
    (it "unquotes function-quoted forms"
      (expect (nucleus-unquote '(function a)) :to-be 'a))
    (it "does nothing to unquoted forms"
      (expect (nucleus-unquote 'hello) :to-be 'hello)))

  (describe "nucleus-enlist"
    (it "creates a list out of non-lists"
      (expect (nucleus-enlist 'a) :to-equal '(a)))
