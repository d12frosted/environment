;;; nucleus-projects.el -*- lexical-binding: t; -*-
...
(def-package! projectile
  ...
  (add-hook 'find-file-hook #'nucleus|init-project-mode)
  (projectile-mode +1)
  ...
  (setq projectile-globally-ignored-directories
        (append projectile-globally-ignored-directories
                (list (abbreviate-file-name nucleus-local-dir) ".sync"
  ...
  (defun nucleus*projectile-locate-dominating-file (orig-fn file name)
    "Don't traverse the file system if on a remote connection."
    (unless (file-remote-p default-directory)
      (funcall orig-fn file name)))
  (advice-add #'projectile-locate-dominating-file :around #'nucleus*projectile-locate-dominating-file))


;;
;; Project-based minor modes

(defvar-local nucleus-project nil
  "Either the symbol or a list of project modes you want to enable. Available
for .dir-locals.el.")

(defvar nucleus-project-hook nil
  "Hook run when a project is enabled. The name of the project's mode and its
state are passed in.")

(defun nucleus|init-project-mode ()
  "Auto-enable the project(s) listed in `nucleus-project'."
  (when nucleus-project
    (if (symbolp nucleus-project)
        (funcall nucleus-project)
      (cl-loop for mode in nucleus-project
...
A project can be enabled through .dir-locals.el too, by setting `nucleus-project'.
  ...
  (let ((init-var (intern (format "%s-init" name))))
    `(progn
       ...
       (define-minor-mode ,name
         ...
         (if (not ,name)
             ,on-exit
           (run-hook-with-args 'nucleus-project-hook ',name ,name)
           ,(when on-load
              `(unless ,init-var
                 ,on-load
                 (setq ,init-var t)))
...
(provide 'nucleus-projects)
;;; nucleus-projects.el ends here
