...
;;;###autoload
(defun nucleus-active-minor-modes ()
          ...
          (setting
           (completing-read
            ...
            (cl-loop with maxwidth = (apply #'max (mapcar #'length (mapcar #'symbol-name settings)))
                     ...
                     if (or (get def 'nucleus-module)
                            (nucleus-module-from-path (symbol-file def)))
                     collect
...
`nucleus--module-mode-alist')."
   ...
   (let* ((module
           (cond ((and buffer-file-name
                 ...
                 ((and buffer-file-name
                       (when-let* ((mod (nucleus-module-from-path buffer-file-name)))
                         (format "%s %s" (car mod) (cdr mod)))))
                 ((when-let* ((mod (cdr (assq major-mode nucleus--module-mode-alist))))
                    (format "%s %s"
                            (symbol-name (car mod))
          ...
          (module-string
           (completing-read
            "Describe module: "
            (cl-loop for path in (nucleus-module-load-path 'all)
                     for (cat . mod) = (nucleus-module-from-path path)
                     for format = (format "%s %s" cat mod)
                     if (nucleus-module-p cat mod)
                     collect format
                     else
  ...
  (or (nucleus-module-p category module)
      (error "'%s %s' isn't a valid module" category module))
  (let ((doc-path (nucleus-module-path category module "README.org")))
