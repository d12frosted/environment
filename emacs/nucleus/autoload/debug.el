;;; debug.el --- the heart of every cell -*- lexical-binding: t; -*-
;;
...
(defun nucleus-template-insert (template)
  "TODO"
  (let ((file (expand-file-name (format "templates/%s" template) nucleus-dir)))
...
(defun nucleus/open-vanilla-sandbox ()
  "Open an Emacs Lisp buffer destinated to run in a blank Emacs
session (and optionally load only nucleus and its modules,
without your private config).

This provides a testbed for debugging code without nucleus (or
your private config) standing in the way, and without sacrificing
