...
(defvar nucleus-cache-alists '(t)
  "An alist of alists, containing lists of variables for the nucleus cache library
...
`nucleus-cache-alists', thus preventing them from being saved between sessions.
...
(defun nucleus-cache-get (key &optional location)
  "Retrieve KEY from LOCATION (defaults to `nucleus-cache-location'), if it exists
and hasn't expired."
...
default is `nucleus-cache-location'. "
...
(defun nucleus-cache-exists (key &optional location)
  "Returns t if KEY exists at LOCATION (defaults to `nucleus-cache-location')."
  (persistent-soft-exists-p key (or location nucleus-cache-location)))

;;;###autoload
(defun nucleus-cache-clear (&optional location)
  "Clear a cache LOCATION (defaults to `nucleus-cache-location')."
