...
;;;###autoload
(defun nucleus/backward-kill-to-bol-and-indent ()
         ...
         (cond ((and (memq (char-before) (list ?\  ?\t))
                     (save-excursion
                       (and (> (- (skip-chars-backward " \t" (line-beginning-position))) 0)
                            (bolp))))
                (nucleus/backward-delete-whitespace-to-column))
               ((let* ((pair (ignore-errors (sp-get-thing)))
                  ...
                  (cond ((and end beg (= end (+ beg (length op) (length cl))))
                        ...
                        ((nucleus-surrounded-p pair 'inline 'balanced)
                         (delete-char -1 killflag)
                         (delete-char 1)
                         (when (= (point) (+ (length cl) beg))
                           (sp-backward-delete-char 1)
                           (sp-insert-pair op)))
                        ((and (bolp) (nucleus-surrounded-p pair nil 'balanced))
                         (delete-region beg end)
                         (sp-insert-pair op)
                         t)
                        ((run-hook-with-args-until-success 'nucleus-delete-backward-functions))
                        ((nucleus/backward-delete-whitespace-to-column)))))))
...
(defvar-local nucleus--buffer-narrowed-origin nil)
;;;###autoload
  ...
  (cond ((or (region-active-p)
         ...
         (when clone-p
           (let ((old-buf (current-buffer)))
             ...
             (setq nucleus--buffer-narrowed-origin old-buf)))
         (narrow-to-region beg end))
        (nucleus--buffer-narrowed-origin
         (kill-this-buffer)
         (switch-to-buffer nucleus--buffer-narrowed-origin)
         (setq nucleus--buffer-narrowed-origin nil))
        (t
         (widen))))

;;;###autoload
(defun nucleus/delete-trailing-newlines ()
...
;;;###autoload
(defun nucleus|enable-delete-trailing-whitespace ()
...
;;;###autoload
(defun nucleus|disable-delete-trailing-whitespace ()
