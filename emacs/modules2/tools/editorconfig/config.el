...
`nucleus*editorconfig-smart-detection' to give editorconfig filetype hints.")
...
(def-package! editorconfig
  ...
  (defun nucleus*editorconfig-smart-detection (orig-fn)
           ...
           (if (and (not (bound-and-true-p org-src-mode))
             ...
             (format "%s%s" buffer-file-name
                     (if-let* ((ext (cdr (assq major-mode +editorconfig-mode-alist))))
                       ...
                       "")))))
      (funcall orig-fn)))
  (advice-add #'editorconfig-call-editorconfig-exec :around #'nucleus*editorconfig-smart-detection)
...
specified by editorconfig."
    (when (or (gethash 'indent_style props)
      ...
      (setq nucleus-inhibit-indent-detection t)))
