...
;; to change. It also isn't feature complete, compared to :ui nucleus-modeline, but
...
(defface nucleus-modeline-buffer-project-root
  '((t (:inherit nucleus-modeline-buffer-path)))
  "Face used for the project root at the beginning of the mode-line path."
  :group '+modeline)

(defface nucleus-modeline-buffer-modified '((t (:inherit (error bold) :background nil)))
  "Face used for the 'unsaved' symbol in the mode-line."
  :group '+modeline)

(defface nucleus-modeline-buffer-major-mode '((t (:inherit (mode-line-emphasis bold))))
  "Face used for the major-mode segment in the mode-line."
  :group '+modeline)

(defface nucleus-modeline-highlight '((t (:inherit mode-line-emphasis)))
  "Face for bright segments of the mode-line."
  :group '+modeline)

(defface nucleus-modeline-panel '((t (:inherit mode-line-highlight)))
  "Face for 'X out of Y' segments, such as `+modeline--anzu',
`+modeline--evil-substitute' and `iedit'"
  :group '+modeline)

(defface nucleus-modeline-info `((t (:inherit (success bold))))
  "Face for info-level messages in the modeline. Used by `*vc'."
  :group '+modeline)

(defface nucleus-modeline-warning `((t (:inherit (warning bold))))
...
use `buffer-name'."
  (let ((buffer-file-name (or path buffer-file-name)))
    (if (or (eq major-mode 'dired-mode)
        ...
        (propertize "%s" 'face 'nucleus-modeline-buffer-path)
...
e.g. project/src/lib/file.c"
  (let* ((base (buffer-base-buffer))
    ...
    (append (if (nucleus-project-p)
                (let* ((project-root (nucleus-project-root))
                       (relative-dirs (file-relative-name (file-name-directory filename)
                                                          (file-truename project-root))))
                  (list (cons (concat (nucleus-project-name) "/")
                              'nucleus-modeline-buffer-project-root)
                        (unless (equal "./" relative-dirs)
                          (cons relative-dirs 'nucleus-modeline-buffer-path))))
              (list nil (cons (abbreviate-file-name (file-name-directory filename))
                              'nucleus-modeline-buffer-path)))
  ...
  (pcase-let
      ((`(,root-parent ,root ,dir, file)
        (let ((buffer-file-name (or buffer-file-name (buffer-file-name (buffer-base-buffer)))))
          (shrink-path-file-mixed (or (nucleus-project-root) default-directory)
                                  (file-name-directory buffer-file-name)
                                  buffer-file-name))))
    (list (cons root-parent 'font-lock-comment-face)
          (cons root 'nucleus-modeline-buffer-project-root)
          (cons (concat "/" dir) 'nucleus-modeline-buffer-path)
          (cons file 'nucleus-modeline-buffer-file))))
...
Meant for `+modeline-buffer-path-function'."
  (pcase-let ((`(,dir . ,file)
    ...
    (list (cons dir 'nucleus-modeline-buffer-path)
          (cons file 'nucleus-modeline-buffer-file))))
...
Meant for `+modeline-buffer-path-function'."
  (list (cons "%b" 'nucleus-modeline-buffer-path)))
  ...
  (setq +modeline-bar-active
        (+modeline--make-xpm +modeline-width +modeline-height
                             (unless +modeline-bar-invisible
                               (face-background 'nucleus-modeline-bar nil t)))
...
(def-modeline-segment! +modeline-buffer-id
  ...
  :init (propertize "%b" 'face 'nucleus-modeline-buffer-file)
  :faces t
...
;;
(set-modeline! :main t)
(add-hook! '+nucleus-dashboard-mode-hook (set-modeline! :project))
(add-hook! 'nucleus-scratch-buffer-hook  (set-modeline! :special))
