...
(def-package! realgud
  ...
  ;; (def-tmp-excmd! nucleus:def-debug-on nucleus:def-debug-off
  ;;   ("n[ext]" . realgud:cmd-next)
  ;;   ("s[tep]" . realgud:cmd-step)
  ;;   ("b[reak]" . +debug:toggle-breakpoint)
  ;;   ("c[ontinue]" . realgud:cmd-continue))
  ;; (advice-add #'realgud-cmdbuf-init :after #'nucleus:def-debug-on)
  ;; (advice-add #'realgud:cmd-quit :after #'nucleus:def-debug-off)
