        ...
        (if (not (member name workspaces))
          ...
          (cond ((delq (selected-frame) (persp-frames-with-persp (get-frame-persp)))
                ...
                ((> (length workspaces) 1)
                 ...
                 (unless (nucleus-buffer-frame-predicate (current-buffer))
                   (switch-to-buffer (nucleus-fallback-buffer))))
                (t
                 (+workspace-switch +workspaces-main t)
                 (unless (string= (car workspaces) +workspaces-main)
                   (+workspace-delete name))
                 (nucleus/kill-all-buffers)))
  ...
  (when persp-mode
    (if (not (persp-frame-list-without-daemon))
      ...
      (with-selected-frame frame
        ...
        (unless (nucleus-real-buffer-p (current-buffer))
          (switch-to-buffer (nucleus-fallback-buffer)))
  ...
  (when (and persp-mode +workspaces--project-dir)
    (unwind-protect
        (if (and (not (null +workspaces-on-switch-project-behavior))
            ...
            (let* (persp-p
                   (persp
                    (let ((project-name (nucleus-project-name +workspaces--project-dir)))
              ...
              (+workspace-message
               ...
               'success))
          (with-current-buffer (switch-to-buffer (nucleus-fallback-buffer))
            (setq default-directory +workspaces--project-dir)
            (message "Switched to '%s'" (nucleus-project-name +workspaces--project-dir)))
          (unless current-prefix-arg
            (funcall +workspaces-switch-project-function +workspaces--project-dir)))
