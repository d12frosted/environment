...
(defun +rss|cleanup ()
  ...
  (let ((buf (previous-buffer)))
    (when (or (null buf) (not (nucleus-real-buffer-p buf)))
      (switch-to-buffer (nucleus-fallback-buffer))))
  (let ((search-buffers (nucleus-buffers-in-mode 'elfeed-search-mode))
        (show-buffers (nucleus-buffers-in-mode 'elfeed-show-mode))
