...
3. Life cycle of configurations are install, upgrade and test. They are executed
   using =./setup.sh= script (or via [[../bootstrap.sh][eru]] from the dotfiles root). See [[*Installation][Installation]]
   section for details.
4. [[https://github.com/progfolio/elpaca][elpaca]] is used as a package manager instead of =package.el=, supporting
   ELPA, MELPA, and git repositories. It provides fast, reliable package
   installation with excellent integration with =use-package=.
...
The =setup.sh= script supports the following commands:
...
The configuration follows a clear structure to maintain fast loading and clean separation:

*** File Categories

1. *=init-*.el=* files - Feature configurations
   - Configure built-in and external packages via =use-package=
   - Must not eagerly load packages that should be lazy-loaded
   - All =init= files are required by =init.el=
   - Example: =init-editor.el=, =init-ui.el=

2. *=config-*.el=* files - Configuration constants
   - Contain variables and constants
   - Safe to require from anywhere (minimal loading time impact)
   - Example: =config-path.el=

3. *=lib-*.el=* files - Autoloaded libraries
   - Should not be required directly
   - Loaded via autoloads generated during build
   - Can freely require packages for byte-compilation
   - Example: =lib-vulpea.el=, =lib-vino.el=

*** Dependencies Between Files

- =init-*.el= can require other =init-*.el= files
- =init-*.el= can require =config-*.el= files
- =lib-*.el= functions are available via autoloads
- All files have access to =lisp/= directory in load-path

** Eldev Integration

[[https://github.com/doublep/eldev/][Eldev]] manages the entire development lifecycle. The =Eldev= file in the project root configures:

*** Build Process

- Bootstraps elpaca and installs all packages
- Generates autoloads from =lib-*.el= files
- Byte-compiles all Emacs Lisp files with warnings as errors

*** Linting

Linting is configured to work with personal configurations (not ELPA packages):

- *checkdoc* - Documentation style checking
- *regexp linter* - Regular expression validation
- *package-lint* is disabled (designed for packages, not configs)
- Custom =.dir-locals.el= with =byte-compile-error-on-warn=

The linter understands the project structure:
- =init-*.el= files can find each other via =lisp/= in load-path
- =lib-*.el= files are checked without prefix requirements
- Non-standard separators allowed (e.g., =vino/germany/wein=)

*** Testing

Uses [[https://github.com/jorgenschaefer/emacs-buttercup][buttercup]] as the testing framework:
...
The configuration includes smart flycheck integration that:

1. *Detects Eldev projects* - Automatically when =Eldev= file is present
2. *Adds project load-path* - =lisp/= directory is available
3. *Includes elpaca packages* - All installed packages are in load-path
4. *No false errors* - =init-*.el= can find each other, =lib-*.el= can find dependencies

This means when editing:
- =init-ide.el= can =(require 'config-path)= without errors
- =lib-brb.el= can =(require 'dash)= without errors
- =lib-vulpea.el= can use =org-roam= functions without errors

*** Autoloads

Functions in =lib-*.el= files marked with =;;;###autoload= are automatically available:

#+begin_src emacs-lisp
;;;###autoload
(defun my-public-function ()
  "This function is available without requiring the file."
  ...)
#+end_src

Autoloads are regenerated by:
- =./setup.sh install= - During installation
- =make bootstrap= - Via Eldev build process
...
(use-package new-package
  :ensure t
  :config
  (setq new-package-option t))
#+end_src

*** Creating a New Library

1. Create =lib-my-feature.el= in =lisp/=
2. Add =;;;###autoload= to public functions
3. Run =./setup.sh install= to regenerate autoloads
4. Functions are now available globally
...
Checks:
- Autoloads file exists
- Elpaca is installed
- Eldev is available
- Emacs version is compatible
- Critical files are present

** Troubleshooting

*** Flycheck shows "Cannot open load file"

1. Ensure packages are installed: =./setup.sh install=
2. Restart Emacs to reload flycheck configuration
3. Check =M-x flycheck-verify-setup= for diagnostics

*** Byte-compilation fails

1. Check if autoloads are generated: =ls lisp/init-autoloads.el=
2. Ensure all dependencies in =use-package= declarations have =:ensure t=
3. Run =./setup.sh clean install= for a fresh build
...
- Regexp syntax
- *Not* package.el conventions (prefix naming, etc.)

If you see package-lint-style errors, they should have been disabled.
...
The configuration uses [[https://github.com/progfolio/elpaca][elpaca]] as the package manager, which provides:

- Fast, asynchronous package installation
- Support for ELPA, MELPA, and git repositories
- Native =use-package= integration via =elpaca-use-package-mode=
- Ability to install packages from personal git repositories

The package management is configured in =init-elpa.el= with bootstrap logic in
the =Eldev= file for CI/development workflows.
