...
declaration. Set by `nucleus-initialize-packages'.")
...
(defun nucleus-assemble-packages (&optional force-p)
  "Ensures that Nucleus's package management system, package.el
and quelpa are initialized, and `nucleus-packages',
`packages-alist' and `quelpa-cache' are populated, if they aren't
already.

If FORCE-P is non-nil, do it anyway.
If FORCE-P is 'internal, only (re)populate `nucleus-packages'.

Use this before any of package.el, quelpa or Nucleus's package
management's API to ensure all the necessary package metadata is
    ...
    (let ((load-prefer-newer t)) ; reduce stale code issues
      ...
      (unless (eq force-p 'internal)
        ...
        (when (or force-p (not (bound-and-true-p package-alist)))
          (nucleus-ensure-packages-initialised 'force)
...
(defun nucleus-ensure-packages-initialised (&optional force-p)
...
populate `nucleus-packages' with metadata about the packages Nucleus needs to keep
track of.
