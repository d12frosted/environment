;;; nucleus-message.el --- the heart of every cell -*- lexical-binding: t; -*-
;;
    ...
    (if noninteractive
      ...
      (propertize
       ...
       (append (get-text-property 0 'face text)
               (cond ((>= code 40)
                      `(:background ,(caddr (assq style nucleus-ansi-alist))))
                     ((>= code 30)
                      `(:foreground ,(face-foreground (caddr (assq style nucleus-ansi-alist)))))
                     ((cddr (assq style nucleus-ansi-alist)))))))))
...
into faces or ANSI codes depending on the type of sesssion we're in."
  `(cl-flet
       (,@(mapcar (lambda (rule) `(,(car rule)
                              (lambda (message)
                                (nucleus-ansi-apply ',(car rule) message))))
                  nucleus-ansi-alist)
        (color
         (lambda (code format)
           (nucleus-ansi-apply code format))))
     (format ,message ,@args)))
...
(provide 'nucleus-message)

;;; nucleus-message.el ends here
