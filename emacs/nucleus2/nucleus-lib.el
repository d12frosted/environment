...
(defun nucleus--resolve-path-forms (spec &optional directory)
  "Converts a simple nested series of or/and forms into a series
of `file-exists-p' checks.

For example

  (nucleus--resolve-path-forms
        ...
        ((and (listp spec)
         ...
         `(,(car spec)
           ,@(cl-loop for i in (cdr spec)
                      collect (nucleus--resolve-path-forms i directory))))
        ((or (symbolp spec)
             (listp spec))
...
(defun nucleus--resolve-hook-forms (hooks)
  (declare (pure t) (side-effect-free t))
  (cl-loop with quoted-p = (eq (car-safe hooks) 'quote)
           for hook in (nucleus-enlist (nucleus-unquote hooks))
...
(defun nucleus--assert-stage-p (stage macro)
  (cl-assert (eq stage nucleus--stage)
             nil
             "Found %s call in non-%s.el file (%s)"
             macro (symbol-name stage)
             (let ((path (FILE!)))
               (if (file-in-directory-p path nucleus-emacs-dir)
...
(defun nucleus-enlist (exp)
  "Return EXP wrapped in a list, or as-is if already a list."
  (declare (pure t) (side-effect-free t))
  (if (listp exp) exp (list exp)))

(defun nucleus-keyword-intern (str)
...
alternative to `after!'."
  ...
  `(if ,condition
       (progn ,@body)
     ,(let ((fun (make-symbol "nucleus|delay-form-")))
        `(progn
...
have been disabled by the user."
  ...
  (unless (and (symbolp targets)
    ...
    (list (if (or (not (bound-and-true-p byte-compile-current-file))
                  (dolist (next (nucleus-enlist targets))
  ...
  `(cond (noninteractive
          (let ((old-fn (symbol-function 'write-region)))
            (cl-letf ((standard-output (lambda (&rest _)))
              ...
              ,@forms)))
         ((or nucleus-debug-mode debug-on-error debug-on-quit)
          ,@forms)
...
function (which will be advised)."
  ...
  (let ((append (if (eq (car forms) :after) (pop forms)))
        (fn (if (symbolp (car forms))
                (intern (format "nucleus|transient-hook-%s" (pop forms)))
              (make-symbol "nucleus|transient-hook-"))))
    `(progn
       (fset ',fn
             (lambda (&rest _)
               ,@forms
...
variable `args'."
  ...
  (let ((hook-fn 'add-hook)
    ...
    (let ((hooks (nucleus--resolve-hook-forms (pop args)))
          (funcs
           (let ((val (car args)))
...
The available conditions are:
  ...
  (unless noninteractive
    (cond ((or files modes when)
           ...
           (let ((hook-name (intern (format "nucleus--init-mode-%s" mode))))
             `(progn
                (fset ',hook-name
                      (lambda ()
                        (and (fboundp ',mode)
                ...
                ,@(if (and modes (listp modes))
                      (cl-loop for hook in (nucleus--resolve-hook-forms modes)
                               collect `(add-hook ',hook #',hook-name))
                    `((add-hook 'after-change-major-mode-hook #',hook-name))))))
          (match
           `(add-to-list 'nucleus-auto-minor-mode-alist '(,match . ,mode)))
...
For example:
  ...
  (if directory
    ...
    (nucleus--resolve-path-forms spec)))
...
exist."
  ...
  (let ((file (if path `(expand-file-name ,filename ,path) filename)))
    `(condition-case e
         (load ,file ,noerror ,(not nucleus-debug-mode))
