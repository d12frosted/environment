...
tests
  = testGroup "BuildMap"
  [ testGroup "buildMap"
    [ testProperty "depending on no resources leads to empty build plan"
      prop_emptyRequirements

    , testProperty "unsatisfied resources are marked as orphans"
      prop_orphans

    , testProperty "ambiguously satisfied resources are marked as such"
      prop_ambiguous
    ]

  , testGroup "buildMapDeep"
    [ testProperty "only dependencies of used providers are added to the final build plan"
      prop_providerDeps

    , testProperty "cyclic dependencies don't affect execution"
      prop_cyclic
    ]
  ]
...
prop_providerDeps resource = monadicIO $ do
  ...
  let pset = HS.fromList providers
  pre $ not (HS.member provider pset)
  bm <- run . runSimpleApp $ buildMapDeep
