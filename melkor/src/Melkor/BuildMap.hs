...
import           Melkor.Types.Internal.SingleRelMap (SingleRelMap,
                                                     mkSingleRelMap)
...
buildMapRaw :: ( HasContext env )
            => HashSet Resource
            -> HashSet Provider
            -> RIO env (HashMap Resource (HashSet Provider))
buildMapRaw rset pset = HM.fromList <$>
