...
data SingleRelMap a b = SingleRelMap
  { _rel :: HashMap a b,
...
uncook srm = foldr (\res -> HM.insert res (providers res)) HM.empty (keys srm)
  where
    providers res =
      HS.unions
        [ HS.fromList $ maybeToList (HM.lookup res (rel srm)),
  ...
  display SingleRelMap {..} =
    mconcat
      [ "{ ",
        ...
        " ambiguous ",
        display . HM.toList . HM.map toList $ _ambiguous,
        " orphans ",
        display $ HS.toList _orphans,
        " }"
      ]
